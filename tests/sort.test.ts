...
describe('(sort) NUMBER sorting', () => {
    test('1 - descending order', () => {
        expect(arrayColumns(sort(input, [['param5', 1]]), 'id')).toEqual([3, 1, 2])
    })

    test('1 - ascending order', () => {
        expect(arrayColumns(sort(input, [['param5', 2]]), 'id')).toEqual([2, 1, 3])
    })
...
describe("(sort) BOOLEAN sorting (FALSE problem, is a falsy value and it's treated as undefined or null)", () => {
    test('1 - descending order', () => {
        expect(arrayColumns(sort(input, [['param3', 1]]), 'id')).toEqual([1, 3, 2])
    })

    test('1 - ascending order', () => {
        expect(arrayColumns(sort(input, [['param3', 2]]), 'id')).toEqual([1, 3, 2])
    })
})

describe('(sort) STRING sorting', () => {
    test('1 - descending order', () => {
        expect(arrayColumns(sort(input, [['param2', 1]]), 'id')).toEqual([3, 2, 1])
    })
...
describe('(sort) SPECIAL sorting', () => {
    test('1 - array (length)', () => {
        expect(arrayColumns(sort(input, [['param6', 1]]), 'id')).toEqual([2, 1, 3])
    })

    test('1 - missing parameter ascending', () => {
        expect(arrayColumns(sort(input, [['param7', 2]]), 'id')).toEqual([2, 1, 3])
    })

    test('1 - missing parameter descending', () => {
        expect(arrayColumns(sort(input, [['param7', 1]]), 'id')).toEqual([1, 2, 3])
    })
})
